...
export const useSyncFiltersFromURL = () => {
  ...
  useEffect(() => {
    ...
    const rating = searchParams.get('rating')?.split(',').map(Number) as [number, number] | undefined
    const years = searchParams.get('years')?.split(',').map(Number) as [number, number] | undefined
